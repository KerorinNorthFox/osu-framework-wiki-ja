...
- **`Sample`**: The sample, which is typically stored by the `Drawable` object in some fashion - either as the raw `Sample` or nested inside a `DrawableSample` object. Used in order to retrieve and/or play one or more `SampleChannel`s.
- **`SampleChannel`**: The object which plays audio. This is typically fired-and-forgotten, with special cases in-case looping or stopping audio playback is required.
